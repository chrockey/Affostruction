...
def _fill_holes(
    ...
    for i in tqdm(
        range(views.shape[0]), total=views.shape[0], disable=not verbose, desc="Rasterizing"
    ):
        view = views[i]
    ...
    g.add_edges(
        ...
        attributes={
            "weight": torch.ones(inner_face_indices.shape[0], dtype=torch.float32).cpu().numpy()
        },
    )
    ...
    g.add_edges(
        ...
        attributes={
            "weight": torch.ones(outer_face_indices.shape[0], dtype=torch.float32).cpu().numpy()
        },
    )
...
def bake_texture(
    ...
    if mode == "fast":
        ...
        total_steps = 2500
        with tqdm(
            total=total_steps, disable=not verbose, desc="Texture baking (opt): optimizing"
        ) as pbar:

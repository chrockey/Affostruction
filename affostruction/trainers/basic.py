...
from ..utils import grad_clip, elastic
...
class BasicTrainer(Trainer):
    ...
    def init_models_and_more(self, **kwargs):
        ...
        if self.elastic_controller_config is not None:
            assert any(
                [
                    isinstance(
                        model, (elastic.ElasticModule, elastic.ElasticModuleMixin)
                    )
            ...
            self.elastic_controller = getattr(
                elastic, self.elastic_controller_config["name"]
        ...
        if self.grad_clip is not None:
            if isinstance(self.grad_clip, (float, int)):
                ...
                self.grad_clip = getattr(grad_clip, self.grad_clip["name"])(
                    **self.grad_clip["args"]
                )
    ...
    def load(self, load_dir, step=0):
        ...
        model_ckpts = {}
        for name, model in self.models.items():
            model_ckpt_path = os.path.join(load_dir, "ckpts", f"{name}_step{step:07d}.pt")
            model_ckpt = torch.load(
                read_file_dist(model_ckpt_path), map_location=self.device, weights_only=True
            )
        ...
        if not self.reset_step:
            ...
            if "val_metric_best" in misc_ckpt and self.is_master:
                self.val_metric_best = misc_ckpt["val_metric_best"]
            ...
            if self.is_master:
                print(f"\n[reset_step=True] Resetting for fine-tuning:")
                print(f"  - Step counter: 0 (not loaded from checkpoint)")
                print(f"  - Optimizer: fresh initialization")
                print(f"  - Data sampler: starting from beginning")
            self.step = 0
    ...
    def save(self, current_val_metric=None):
        ...
        if self.save_best_only and current_val_metric is not None:
            is_better = False
            if self.validation_config["metric_mode"] == "max":
                is_better = current_val_metric > self.val_metric_best
            else:
                is_better = current_val_metric < self.val_metric_best

            if not is_better:
                print(
                    f"\nSkipping checkpoint save at step {self.step} (current {current_val_metric:.6f} | best {self.val_metric_best:.6f})"
                )
                return

            stale_ckpts = glob.glob(os.path.join(self.output_dir, "ckpts", "*.pt"))

            self.val_metric_best = current_val_metric
            checkpoint_suffix = f"step{self.step:07d}"
            print(
                f"\nSaving new best checkpoint at step {self.step} (current {current_val_metric:.6f} | best {self.val_metric_best:.6f})...",
                end="",
            )
        else:
            stale_ckpts = []
        ...
        for name, model_ckpt in model_ckpts.items():
            torch.save(
                model_ckpt,
                os.path.join(self.output_dir, "ckpts", f"{name}_{checkpoint_suffix}.pt"),
            )
        ...
        for i, ema_rate in enumerate(self.ema_rate):
            ...
            for name, ema_ckpt in ema_ckpts.items():
                torch.save(
                    ema_ckpt,
                    os.path.join(
                        self.output_dir, "ckpts", f"{name}_ema{ema_rate}_{checkpoint_suffix}.pt"
                    ),
                )
        ...
        if self.save_best_only and current_val_metric is not None:
            misc_ckpt["val_metric_best"] = self.val_metric_best
        ...
        torch.save(
            misc_ckpt, os.path.join(self.output_dir, "ckpts", f"misc_{checkpoint_suffix}.pt")
        )

        for file in stale_ckpts:
            if checkpoint_suffix not in os.path.basename(file):
                os.remove(file)
    ...
    def run_step(self, data_list):
        ...
        amp_context = (
            partial(torch.autocast, device_type="cuda") if self.fp16_mode == "amp" else nullcontext
        )
        ...
        for i, mb_data in enumerate(data_list):
            ...
            with nested_contexts(*sync_contexts), elastic_controller_context():
                ...
                if self.fp16_mode == "amp":
                    ...
                    l.backward()
            losses.append(
                dict_foreach(loss, lambda x: x.detach() if isinstance(x, torch.Tensor) else x)
            )
            statuses.append(
                dict_foreach(status, lambda x: x.detach() if isinstance(x, torch.Tensor) else x)
            )
            if self.elastic_controller_config is not None:
        ...
        if self.grad_clip is not None:
            ...
            if torch.isfinite(grad_norm):
                statuses[-1]["grad_norm"] = grad_norm.detach()
        if self.fp16_mode == "amp":
        ...
        def _mean_with_tensor(x):
            return np.mean([_to_scalar(v) for v in x])

        def _min_with_tensor(x):
            return np.min([_to_scalar(v) for v in x])

        def _max_with_tensor(x):
            return np.max([_to_scalar(v) for v in x])

        step_log["loss"] = dict_reduce(losses, _mean_with_tensor)
        step_log["status"] = dict_reduce(
            statuses,
            _mean_with_tensor,
            special_func={"min": _min_with_tensor, "max": _max_with_tensor},
        )
        if self.elastic_controller_config is not None:
            step_log["elastic"] = dict_reduce(elastic_controller_logs, lambda x: np.mean(x))
        if self.grad_clip is not None:
            step_log["grad_clip"] = (
                self.grad_clip if isinstance(self.grad_clip, float) else self.grad_clip.log()
            )
